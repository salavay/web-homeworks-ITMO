...
public class UserRepositoryImpl extends AbstractRepository<UserRepositoryImpl.UserWithPasswordsha> implements UserRepository {
    ...
    private final String changeAdminByIdStatement = "UPDATE User SET admin=? WHERE id=?";
    ...
    private void changeAdminByIdStatementSetter(UserWithPasswordsha user,
                                                PreparedStatement preparedStatement) throws SQLException {
        preparedStatement.setBoolean(1, user.getUser().getAdmin());
        preparedStatement.setLong(2, user.getUser().getId());
    }
    ...
    public User changeAdminById(long id, boolean newIsAdmin) {
        UserWithPasswordsha userWithPasswordsha = new UserWithPasswordsha(find(id), "");
        userWithPasswordsha.getUser().setAdmin(newIsAdmin);
        super.update(userWithPasswordsha, changeAdminByIdStatement, this::changeAdminByIdStatementSetter);
        return safeUnpackUser(userWithPasswordsha);
    }
