  ...
  beforeCreate() {
    this.$root.$on("onRegister", (login, name, password) => {
      ...
      if (login.length < 3 || login.length > 16) {
        this.$root.$emit("onRegisterValidationError", "Login have to be from 3 to 16 characters in size");
        return;
      }

      if (/^[a-z]+$/.test(login) === false) {
        this.$root.$emit("onRegisterValidationError", "Login can contain only lowercase letters");
        return;
      }

      if (name.length < 1 || name.length > 32) {
        this.$root.$emit("onRegisterValidationError", "Name have to be from 1 to 32 characters in size");
        return;
      }
